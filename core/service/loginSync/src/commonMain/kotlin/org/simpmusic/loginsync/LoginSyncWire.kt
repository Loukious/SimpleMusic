...
internal object LoginSyncWire {
    ...
    fun aad(
        kind: String,
        reply: Boolean,
    ): ByteArray = "simplemusic-login-sync/v1/$kind/${if (reply) "reply" else "request"}".encodeToByteArray()
}
...
/** What the QR carries: `simplemusic://login-sync?h=<ip>&p=<port>&k=<base64url key>`. */
