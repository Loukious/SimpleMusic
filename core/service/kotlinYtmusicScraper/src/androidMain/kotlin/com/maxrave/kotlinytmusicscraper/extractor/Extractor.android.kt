...
actual class Extractor {
    ...
    actual fun mergeAudioVideoDownload(filePath: String): DownloadProgress {
        val command =
            listOf(
                ...
                "1:a:0",
                "-shortest",
                "$filePath-SimpleMusic.mp4",
            ).joinToString(" ")

        if (FileSystem.SYSTEM.exists("$filePath-SimpleMusic.mp4".toPath())) {
            FileSystem.SYSTEM.delete("$filePath-SimpleMusic.mp4".toPath())
        }
    ...
    actual fun saveAudioWithThumbnail(
        ...
        try {
            ...
            if (FileSystem.SYSTEM.exists("$filePath-simplemusic.mp3".toPath())) {
                FileSystem.SYSTEM.delete("$filePath-simplemusic.mp3".toPath())
            }
        } catch (e: IOException) {
            e.printStackTrace()
        }
