@echo off
REM SimpleMusic offline installer for Windows.
...
REM   - simplemusic.crt
REM   - simplemusic-<version>.msix     (any .msix matching simplemusic-*.msix)
REM
...
echo.
echo === SimpleMusic offline installer ===
echo.

REM --- 2. Install cert to Trusted People (idempotent via certutil) -----------
if not exist "%~dp0simpmusic.crt" (
    echo [ERROR] simplemusic.crt not found next to install.bat
    pause
    exit /b 1
)

echo [1/3] Importing simplemusic.crt to LocalMachine\TrustedPeople ...
...
if not defined MSIX (
    echo [ERROR] No simplemusic-*.msix found in this folder.
    pause
    exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ForceUpdateFromAnyVersion -ErrorAction Stop; exit 0 } catch { Write-Host 'In-place update failed; removing existing SimpleMusic and reinstalling...'; Get-AppxPackage -Name 'Simpmusic' | Remove-AppxPackage -ErrorAction SilentlyContinue; try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ErrorAction Stop; exit 0 } catch { Write-Host $_; exit 1 } }"
...
REM --- 4. Launch app ---------------------------------------------------------
echo.
echo [3/3] Launching SimpleMusic ...
start "" "shell:appsFolder\Simpmusic_ejp2bhxmz1qq6!Simpmusic"

echo.
echo SimpleMusic installed successfully.
echo To uninstall: Settings -^> Apps -^> Installed apps -^> SimpleMusic -^> Uninstall
echo.
pause
endlocal
